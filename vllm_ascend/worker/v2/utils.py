...
@contextmanager
def torch_npu_graph_wrapper(npu_graph, *args, **kwargs):
    # npugraph_ex runs eagerly inside MRV2's outer graph, so the outer
    # capture must also apply SuperKernel to the target and draft graphs.
    enable_super_kernel = get_ascend_config().ascend_compilation_config.enable_super_kernel
    try:
        with torch.npu.graph(npu_graph, *args, **kwargs), _super_kernel_scope("full_model", enable_super_kernel):
            yield
        if enable_super_kernel:
            npu_graph.super_kernel_optimize(
